...
pub async fn run(addr: &str, recv_timeout: Duration, pb: &ProgressBar) -> Suite {
    Suite {
        ...
        results: vec![
            ...
            qos1_delivery(addr, recv_timeout, pb).await,
            ...
            retained_deletion(addr, recv_timeout, pb).await,
            retained_replacement(addr, recv_timeout, pb).await,
            puback_no_matching_subscribers(addr, recv_timeout, pb).await,
            message_expiry_countdown(addr, recv_timeout, pb).await,
            max_packet_size_enforcement(addr, recv_timeout, pb).await,
            topic_alias_reuse(addr, recv_timeout, pb).await,
...
async fn qos1_delivery(addr: &str, recv_timeout: Duration, pb: &ProgressBar) -> TestResult {
    ...
    run_test(ctx, pb, async {
        ...
        for _ in 0..5 {
            if let Packet::PubAck(_) = pub_client.recv(recv_timeout).await? {
                break;
            }
        }
...
async fn retained_deletion(addr: &str, recv_timeout: Duration, pb: &ProgressBar) -> TestResult {
    ...
    run_test(ctx, pb, async {
        ...
        let mut sub_client = client::connect_and_subscribe(addr, "mqtt-test-retdel-sub", topic, QoS::AtMostOnce, recv_timeout).await?;
...
async fn retained_replacement(addr: &str, recv_timeout: Duration, pb: &ProgressBar) -> TestResult {
    ...
    run_test(ctx, pb, async {
        ...
        let mut sub_client = client::connect_and_subscribe(addr, "mqtt-test-retrpl-sub", topic, QoS::AtMostOnce, recv_timeout).await?;
...
async fn puback_no_matching_subscribers(addr: &str, recv_timeout: Duration, pb: &ProgressBar) -> TestResult {
...
async fn message_expiry_countdown(addr: &str, recv_timeout: Duration, pb: &ProgressBar) -> TestResult {
    ...
    run_test(ctx, pb, async {
        ...
        let (mut sub_client2, _) = client::connect(addr, &sub_params2, recv_timeout).await?;

        match sub_client2.recv(recv_timeout).await {
            Ok(Packet::Publish(p)) if p.topic == topic => {
                if let Some(pid) = p.packet_id {
...
async fn max_packet_size_enforcement(addr: &str, recv_timeout: Duration, pb: &ProgressBar) -> TestResult {
...
async fn topic_alias_reuse(addr: &str, recv_timeout: Duration, pb: &ProgressBar) -> TestResult {
    let ctx = TOPIC_ALIAS_REUSE;
    run_test(ctx, pb, async {
        let topic = "mqtt/test/pub/alias_reuse";

        let params = ConnectParams::new("mqtt-test-alias-reuse-pub");
        let (mut pub_client, connack) = client::connect(addr, &params, recv_timeout).await?;
        ...
        // Set up a subscriber
        let mut sub_client = client::connect_and_subscribe(addr, "mqtt-test-alias-reuse-sub", topic, QoS::AtMostOnce, recv_timeout).await?;
...
async fn topic_alias_reset_on_reconnect(addr: &str, recv_timeout: Duration, pb: &ProgressBar) -> TestResult {
    ...
    run_test(ctx, pb, async {
        ...
        let (mut client2, connack2) = client::connect(addr, &params2, recv_timeout).await?;
        ...
        match client2.recv(recv_timeout).await {
            Err(_) | Ok(Packet::Disconnect(_)) => Ok(TestResult::pass(&ctx)),
...
async fn receive_maximum_flow_control(addr: &str, recv_timeout: Duration, pb: &ProgressBar) -> TestResult {
...
async fn qos2_duplicate_publish(addr: &str, recv_timeout: Duration, pb: &ProgressBar) -> TestResult {
    let ctx = QOS2_DUP;
    run_test(ctx, pb, async {
