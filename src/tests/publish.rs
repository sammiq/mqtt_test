...
pub fn tests<'a>(config: TestConfig<'a>) -> SuiteRunner<'a> {
    ...
    suite.add(RETAIN_DELIVERY_FLAG, retained_delivered_with_retain_flag(config));
    ...
    suite.add(QOS2_EXPIRY_CONTINUES, qos2_continues_after_message_expiry(config));
    ...
    suite.add(RETAIN_ZERO_PRESERVES, retain_zero_preserves_existing(config));
    suite.add(ORDERED_TOPIC_QOS0, ordered_topic_qos0(config));
    suite.add(CONTENT_TYPE_FORWARDED, content_type_forwarded_unaltered(config));
...
async fn qos0_accepted(config: TestConfig<'_>) -> anyhow::Result<TestResult> {
    ...
    let mut client = client::connect_and_subscribe(config.addr, "mqtt-test-qos0-pub", "mqtt/test/pub/qos0", QoS::AtMostOnce, config.recv_timeout).await?;
...
async fn qos1_delivery(config: TestConfig<'_>) -> anyhow::Result<TestResult> {
    ...
    // Subscriber at QoS 1
    let mut sub_client = client::connect_and_subscribe(config.addr, "mqtt-test-qos1-del-sub", topic, QoS::AtLeastOnce, config.recv_timeout).await?;
...
async fn qos_downgrade_on_delivery(config: TestConfig<'_>) -> anyhow::Result<TestResult> {
    ...
    let mut sub_client = client::connect_and_subscribe(config.addr, "mqtt-test-qos-dg-sub", topic, QoS::AtMostOnce, config.recv_timeout).await?;
...
async fn retain_flag_accepted(config: TestConfig<'_>) -> anyhow::Result<TestResult> {
    ...
    let mut sub_client = client::connect_and_subscribe(config.addr, "mqtt-test-retain-sub", "mqtt/test/pub/retain", QoS::AtMostOnce, config.recv_timeout).await?;
...
async fn property_forwarding_test(
    ...
    let mut client = client::connect_and_subscribe(config.addr, &format!("mqtt-test-{topic}"), topic, QoS::AtMostOnce, config.recv_timeout).await?;
...
async fn payload_format_indicator_preserved(config: TestConfig<'_>) -> anyhow::Result<TestResult> {
    ...
    property_forwarding_test(config, PFI, "mqtt/test/pub/pfi", props,
...
async fn message_expiry_interval_present(config: TestConfig<'_>) -> anyhow::Result<TestResult> {
    ...
    property_forwarding_test(config, MEI, "mqtt/test/pub/mei", props,
...
async fn content_type_preserved(config: TestConfig<'_>) -> anyhow::Result<TestResult> {
    ...
    property_forwarding_test(config, CONTENT_TYPE, "mqtt/test/pub/ct", props,
...
async fn response_topic_preserved(config: TestConfig<'_>) -> anyhow::Result<TestResult> {
    ...
    property_forwarding_test(config, RESPONSE_TOPIC, "mqtt/test/pub/rt", props,
...
async fn correlation_data_preserved(config: TestConfig<'_>) -> anyhow::Result<TestResult> {
    ...
    property_forwarding_test(config, CORRELATION_DATA, "mqtt/test/pub/cd", props,
...
async fn user_properties_preserved(config: TestConfig<'_>) -> anyhow::Result<TestResult> {
    ...
    property_forwarding_test(config, USER_PROPS, "mqtt/test/pub/up", props,
...
async fn message_ordering(config: TestConfig<'_>) -> anyhow::Result<TestResult> {
    ...
    // Subscriber at QoS 1
    let mut sub_client = client::connect_and_subscribe(config.addr, "mqtt-test-order-sub", topic, QoS::AtLeastOnce, config.recv_timeout).await?;
...
async fn retained_delivered_with_retain_flag(config: TestConfig<'_>) -> anyhow::Result<TestResult> {
    ...
    let mut sub_client = client::connect_and_subscribe(config.addr, "mqtt-test-retflag-sub", topic, QoS::AtMostOnce, config.recv_timeout).await?;
...
async fn retained_deletion(config: TestConfig<'_>) -> anyhow::Result<TestResult> {
    ...
    let (mut pub_client, _) = client::connect(config.addr, &pub_conn, config.recv_timeout).await?;
    pub_client.send_publish(&PublishParams::retained(topic, b"to-delete".to_vec())).await?;
    tokio::time::sleep(Duration::from_millis(100)).await;

    // Delete it with empty payload + retain
    pub_client.send_publish(&PublishParams::retained(topic, Vec::new())).await?;
    tokio::time::sleep(Duration::from_millis(100)).await;

    // New subscriber should NOT receive any retained message
    let mut sub_client = client::connect_and_subscribe(config.addr, "mqtt-test-retdel-sub", topic, QoS::AtMostOnce, config.recv_timeout).await?;
...
async fn retained_replacement(config: TestConfig<'_>) -> anyhow::Result<TestResult> {
    ...
    // Publish v1
    pub_client.send_publish(&PublishParams::retained(topic, b"v1".to_vec())).await?;
    tokio::time::sleep(Duration::from_millis(100)).await;

    // Publish v2 (should replace v1)
    pub_client.send_publish(&PublishParams::retained(topic, b"v2".to_vec())).await?;
    tokio::time::sleep(Duration::from_millis(100)).await;

    // New subscriber should get v2 only
    let mut sub_client = client::connect_and_subscribe(config.addr, "mqtt-test-retrpl-sub", topic, QoS::AtMostOnce, config.recv_timeout).await?;
    ...
    // Clean up
    let _ = pub_client.send_publish(&PublishParams::retained(topic, Vec::new())).await;
...
async fn puback_no_matching_subscribers(config: TestConfig<'_>) -> anyhow::Result<TestResult> {
    ...
    client.send_publish(&PublishParams::qos1(
    ...
    match client.recv(config.recv_timeout).await? {
        Packet::PubAck(ack) if ack.packet_id == 1 => {
            if ack.reason_code == 0x10 {
                ...
                Ok(TestResult::fail(
                    &ctx,
                    format!("PUBACK reason code {:#04x} (expected 0x10 for no matching subscribers)", ack.reason_code),
                ))
            }
        }
...
async fn message_expiry_countdown(config: TestConfig<'_>) -> anyhow::Result<TestResult> {
    ...
    let pub_params = PublishParams {
        ...
        dup: false,
        packet_id: Some(1),
        properties: Properties { message_expiry_interval: Some(60), ..Properties::default() },
    };
    pub_client.send_publish(&pub_params).await?;
    // Drain PUBACK
    for _ in 0..5 {
    ...
    let (mut sub_client2, _) = client::connect(config.addr, &sub_params2, config.recv_timeout).await?;
    ...
    match sub_client2.recv(config.recv_timeout).await {
        Ok(Packet::Publish(p)) if p.topic == topic => {
            ...
            match p.properties.message_expiry_interval {
                ...
                Some(mei) => Ok(TestResult::fail(&ctx, format!(
                    "MEI not decremented: received {mei}, expected < 60"
                ))),
                None => Ok(TestResult::fail(&ctx, "No Message Expiry Interval in forwarded PUBLISH")),
            }
        }
        Ok(other) => Ok(TestResult::fail_packet(&ctx, "PUBLISH with decremented MEI", &other)),
        Err(_) => Ok(TestResult::fail(&ctx, "No message delivered after session resume")),
    }
...
async fn max_packet_size_enforcement(config: TestConfig<'_>) -> anyhow::Result<TestResult> {
    ...
    pub_client.send_publish(&PublishParams::qos0(topic, big_payload)).await?;
    ...
    if got_big {
        return Ok(TestResult::fail(&ctx, "Server sent packet exceeding client's Maximum Packet Size"));
    }

    // Verify small messages still work
    pub_client.send_publish(&PublishParams::qos0(topic, b"small".to_vec())).await?;
...
async fn topic_alias_reuse(config: TestConfig<'_>) -> anyhow::Result<TestResult> {
    ...
    let (mut pub_client, connack) = client::connect(config.addr, &params, config.recv_timeout).await?;

    let max_alias = connack.properties.topic_alias_maximum.unwrap_or(0);
    if max_alias == 0 {
        return Ok(TestResult::skip(&ctx, "Broker does not support Topic Aliases (maximum = 0)"));
    }

    // Set up a subscriber
    let mut sub_client = client::connect_and_subscribe(config.addr, "mqtt-test-alias-reuse-sub", topic, QoS::AtMostOnce, config.recv_timeout).await?;
    ...
    let p1 = PublishParams {
        ...
        dup: false,
        packet_id: None,
        properties: Properties { topic_alias: Some(1), ..Properties::default() },
    };
    pub_client.send_publish(&p1).await?;

    match sub_client.recv(config.recv_timeout).await? {
        Packet::Publish(p) if p.topic == topic => {}
        other => return Ok(TestResult::fail_packet(&ctx, "first PUBLISH via alias", &other)),
    }
    ...
    match sub_client.recv(config.recv_timeout).await {
        Ok(Packet::Publish(p)) if p.topic == topic => Ok(TestResult::pass(&ctx)),
        Ok(Packet::Publish(p)) => Ok(TestResult::fail(&ctx, format!(
            "Message delivered on wrong topic: {:?}", p.topic
        ))),
        Ok(other) => Ok(TestResult::fail_packet(&ctx, "PUBLISH via alias reuse", &other)),
        Err(_) => Ok(TestResult::fail(&ctx, "No message delivered via alias reuse")),
    }
...
async fn topic_alias_reset_on_reconnect(config: TestConfig<'_>) -> anyhow::Result<TestResult> {
    ...
    let (mut client2, connack2) = client::connect(config.addr, &params2, config.recv_timeout).await?;

    if connack2.properties.topic_alias_maximum.unwrap_or(0) == 0 {
        return Ok(TestResult::skip(&ctx, "Broker does not support Topic Aliases after reconnect"));
    }
    ...
    match client2.recv(config.recv_timeout).await {
        Err(_) | Ok(Packet::Disconnect(_)) => Ok(TestResult::pass(&ctx)),
        Ok(other) => Ok(TestResult::fail_packet(&ctx, "disconnect (alias reset)", &other)),
    }
...
async fn receive_maximum_flow_control(config: TestConfig<'_>) -> anyhow::Result<TestResult> {
    ...
    let (mut pub_client, _) = client::connect(config.addr, &pub_conn, config.recv_timeout).await?;
    for i in 1u16..=5 {
        pub_client.send_publish(&PublishParams::qos1(topic, format!("msg-{i}").into_bytes(), i)).await?;
    }
    // Drain PUBACKs
    for _ in 0..10 {
    ...
    if unacked.len() > 2 {
        return Ok(TestResult::fail(&ctx, format!(
            "Server sent {} unacknowledged messages (receive_maximum=2)", unacked.len()
        )));
    }
...
async fn qos2_duplicate_publish(config: TestConfig<'_>) -> anyhow::Result<TestResult> {
    ...
    for _ in 0..5 {
        match client.recv(config.recv_timeout).await? {
            ...
            Packet::Publish(_) => {} // loopback
            other => return Ok(TestResult::fail_packet(&ctx, "PUBREC(10) after DUP", &other)),
        }
    }
...
async fn packet_id_reuse_after_puback(config: TestConfig<'_>) -> anyhow::Result<TestResult> {
    ...
    let mut sub = client::connect_and_subscribe(config.addr, "mqtt-test-pidq1-sub", topic, QoS::AtLeastOnce, config.recv_timeout).await?;
...
async fn user_properties_order(config: TestConfig<'_>) -> anyhow::Result<TestResult> {
    ...
    let mut sub_client = client::connect_and_subscribe(
        config.addr, "mqtt-test-uporder-sub", topic, QoS::AtMostOnce, config.recv_timeout,
    ).await?;
...
async fn retained_qos0_stored(config: TestConfig<'_>) -> anyhow::Result<TestResult> {
    ...
    let mut sub_client = client::connect_and_subscribe(
        config.addr, "mqtt-test-retq0-sub", topic, QoS::AtMostOnce, config.recv_timeout,
    ).await?;
...
async fn qos2_no_duplicate_delivery(config: TestConfig<'_>) -> anyhow::Result<TestResult> {
    ...
    let mut sub_client = client::connect_and_subscribe(
        config.addr, "mqtt-test-q2nodup-sub", topic, QoS::AtMostOnce, config.recv_timeout,
    ).await?;
...
async fn qos1_initial_delivery_dup_zero(config: TestConfig<'_>) -> anyhow::Result<TestResult> {
    ...
    let mut sub_client = client::connect_and_subscribe(
        config.addr, "mqtt-test-q1dup0-sub", topic, QoS::AtLeastOnce, config.recv_timeout,
    ).await?;
...
async fn retain_zero_preserves_existing(config: TestConfig<'_>) -> anyhow::Result<TestResult> {
    ...
    let mut sub_client = client::connect_and_subscribe(
        config.addr, "mqtt-test-r0p-sub", topic, QoS::AtMostOnce, config.recv_timeout,
    ).await?;
...
async fn ordered_topic_qos0(config: TestConfig<'_>) -> anyhow::Result<TestResult> {
    ...
    // Subscriber at QoS 0
    let mut sub_client = client::connect_and_subscribe(
        config.addr, "mqtt-test-ord0-sub", topic, QoS::AtMostOnce, config.recv_timeout,
    ).await?;
...
async fn content_type_forwarded_unaltered(config: TestConfig<'_>) -> anyhow::Result<TestResult> {
    ...
    // Subscriber
    let mut sub_client = client::connect_and_subscribe(
        config.addr, "mqtt-test-ct-fwd-sub", topic, QoS::AtLeastOnce, config.recv_timeout,
    ).await?;
